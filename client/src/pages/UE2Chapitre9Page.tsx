...
function QCMBlock({ q }: { q: QCMQuestion }) {
  ...
  return (
    <div className="rounded-xl border border-orange-200 dark:border-orange-800 bg-orange-50/30 dark:bg-orange-900/10 p-4 space-y-3">
      <p className="text-xs font-semibold text-orange-700 dark:text-orange-300">{q.question}</p>
      <div className="space-y-1.5">
        {q.options.map(opt => {
          let cls = 'w-full text-left text-xs px-3 py-2 rounded-lg border transition-colors '
          if (!showResult) cls += selected === opt.id ? 'border-orange-500 bg-orange-100 dark:bg-orange-900/30 text-orange-800 dark:text-orange-200' : 'border-border hover:border-orange-300 hover:bg-muted/40'
