...
export default function BalancePage({ embedded = false }: { embedded?: boolean } = {}) {
  const user = useUser()
  ...
  return (
    <div className={cn('space-y-5', !embedded && 'animate-fadeIn')}>
      ...
      {embedded && lignes.length > 0 && (
        <div className="flex justify-end">
          <Button variant="outline" size="sm" onClick={() => exportBalancePDF(selectedSession?.nom || '', lignes)}>
            <Download className="h-4 w-4 mr-1" /> PDF
          </Button>
        </div>
      )}
