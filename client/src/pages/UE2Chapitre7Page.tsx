...
import {
  ...
  Building2, Star, FileText, Globe, ChevronDown, ChevronUp,
...
function QCMBlock({ q }: { q: QCMQuestion }) {
  ...
  return (
    <div className="rounded-xl border border-amber-200 dark:border-amber-800 bg-amber-50/30 dark:bg-amber-900/10 p-4 space-y-3">
      <p className="text-xs font-semibold text-amber-700 dark:text-amber-300">{q.question}</p>
      <div className="space-y-1.5">
        {q.options.map(opt => {
          let cls = 'w-full text-left text-xs px-3 py-2 rounded-lg border transition-colors '
          if (!showResult) cls += selected === opt.id ? 'border-amber-500 bg-amber-100 dark:bg-amber-900/30 text-amber-800 dark:text-amber-200' : 'border-border hover:border-amber-300 hover:bg-muted/40'
